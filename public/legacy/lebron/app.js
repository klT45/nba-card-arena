...
let renderer,composer,bloom,root,face,uniforms,config,auto=false,flipped=false,dragging=false;
let targetX=0.025,targetY=-0.13,targetZoom=1,rotationX=targetX,rotationY=targetY;
let last={x:0,y:0},lastTime=0,elapsed=0;
// 自适应画质：从 maxPR 起步，卡顿自动降分辨率/关辉光，流畅后再升回来。
let maxPR=Math.min(devicePixelRatio||1,1.5),curPR=maxPR,fpsEMA=60,frames=0,lastQ=0;
...
async function init(){
 ...
 renderer=new THREE.WebGLRenderer({antialias:true,alpha:false,preserveDrawingBuffer:false,powerPreference:'high-performance'});renderer.setClearColor(0xffffff,1);renderer.setPixelRatio(curPR);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.12;stage.append(renderer.domElement);
 composer=new EffectComposer(renderer);composer.setPixelRatio(curPR);composer.addPass(new RenderPass(scene,camera));bloom=new UnrealBloomPass(new THREE.Vector2(720,1000),.18,.35,1.0);composer.addPass(bloom);composer.addPass(new OutputPass());
 const loader=new THREE.TextureLoader();const names=['subject','background','text','lineart'];const textures=await Promise.all(names.map(name=>loader.loadAsync(config.assets[name])));textures.forEach(t=>{t.colorSpace=THREE.NoColorSpace;t.anisotropy=Math.min(renderer.capabilities.getMaxAnisotropy(),4);});
...
function resize(){const w=stage.clientWidth,h=stage.clientHeight;if(!w||!h||!renderer)return;const aspect=w/h;const halfH=5.65/targetZoom;camera.left=-halfH*aspect;camera.right=halfH*aspect;camera.top=halfH;camera.bottom=-halfH;camera.updateProjectionMatrix();renderer.setPixelRatio(curPR);composer.setPixelRatio(curPR);renderer.setSize(w,h);composer.setSize(w,h);}
...
function animate(now){const dt=Math.min((now-lastTime)/1000,.1)||0;lastTime=now;if(document.hidden)return;elapsed+=dt;if(auto){targetY=Math.sin(elapsed*.65)*.38;targetX=Math.sin(elapsed*.85)*.12;}
 frames++;if(dt>0)fpsEMA+=(1/dt-fpsEMA)*.04;
 if(frames%90===0){const t=performance.now();if(t-lastQ>4000){lastQ=t;
  if(fpsEMA<45){if(curPR>1){curPR=Math.max(1,curPR-.25);resize();}else if(bloom&&bloom.enabled!==false){bloom.enabled=false;}}
  else if(fpsEMA>57&&(curPR<maxPR||(bloom&&bloom.enabled===false))){if(curPR<maxPR)curPR=Math.min(maxPR,curPR+.25);else bloom.enabled=true;resize();}}}
 ...
 uniforms.uView.value.copy(camera.position).applyMatrix4(_inv.copy(root.matrixWorld).invert()).normalize();uniforms.uTime.value=reduced&&!auto?0:elapsed;composer.render();}
